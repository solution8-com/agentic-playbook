...
8. **Permission-role fixtures**: one login per role the app distinguishes, and how to mint a
   token as each. The permission matrix is only as complete as this roster, so a missing role
...
**fail** (what broke, screenshot the broken state), **blocked** (why), or **accepted**.

**`accepted`** is for a check that reproduces a deferral already ruled on: the behaviour is
known, someone decided not to fix it yet, and the decision reached this run through the brief.
Mark it `accepted` with a pointer to where it was decided. Never `fail`, which re-litigates a
settled call, and never a silent skip, which loses the evidence that the behaviour is still
there. It renders muted and stays out of the verdict.
...
`status` is `pass` | `fail` | `blocked` | `accepted` | `na` | `notes` (use `notes` for
Observations, `na` for untouched categories, `accepted` for a ruled-on deferral). Flows are for screenshot-bearing UI checks; sections for everything else.
